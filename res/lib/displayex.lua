...
for idx, _align in ipairs(ALIGN_TAG) do --labels
    ...
    d["label" .. _align] = function(strvalue, ttf, fontsize, color, width, align)
        local ret
        if not ttf then
            ret = cc.Label:createWithSystemFont(strvalue, "arial", fontsize or d.DEFAULT_TTF_FONT_SIZE)
            if width then
                ret:setWidth(width)
            end
        else
            ret = cc.Label:createWithTTF({
                fontSize = fontsize or d.DEFAULT_TTF_FONT_SIZE,
                fontFilePath = ttf
                },
                strvalue,
                align or cc.VERTICAL_TEXT_ALIGNMENT_CENTER, width or 0)
            ret:setLineBreakWithoutSpace(true)
        end
        function ret:color(_color)
            self:setColor(_color)
            return self
        end

        function ret:outline(color, size)
            self:enableOutline(color, size)
            return self
        end

        --**old ttf has not the shadow and glow method!**
        function ret:shadow(color, offsetx, offsety)
            self:enableShadow(color, cc.size(offsetx, offsety))
            return self
        end
        function ret:glow(color)
